...
const Paises = ({ movies }) => {
  ...
  const countryCounts = movies.reduce((acc, movie) => {
    ...
    if (!country) {
      acc["Desconhecido"] = (acc["Desconhecido"] || 0) + 1;
      return acc;
    }

    // Caso country seja um array
    if (Array.isArray(country)) {
      country.forEach((pais) => {
        const nome = pais.trim();
        acc[nome] = (acc[nome] || 0) + 1;
      });
    }

    // Caso country seja uma string
    else {
      country.split(",").forEach((pais) => {
        const nome = pais.trim();
        acc[nome] = (acc[nome] || 0) + 1;
      });
    }
              ...
              <div key={item.name} className="pais-bar-container">
                ...
                <div className="pais-bar-bg">
                  <div
                    ...
                    style={{
                      width: `${(item.count / maxCountry) * 100}%`
                    }}
                  ></div>
                </div>
              ...
              <div key={item.name} className="pais-bar-container">
                ...
                <div className="pais-bar-bg">
                  <div
                    className="pais-bar-fill"
                    style={{
                      width: `${(item.count / maxLanguage) * 100}%`
                    }}
                  ></div>
                </div>
